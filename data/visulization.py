...
def plot2(ylabel, yticks=None):
    ...
    if yticks is not None:
        plt.yticks(yticks)
    plt.xlabel("Patience Duration (Minute)")
    plt.ylabel(ylabel)
    plt.legend()
    plt.show()
...
soft_cdqn = [81, 88, 91.2, 92.5, 93]
#bsac_nn = [81.5, 89.4, 92.8, 94.7, 95.86]
bsac_rnn_pdsa = [83, 90.5, 94.32, 95.84, 97]
plot1("Service Rate (%)", [50, 60, 70, 80, 90, 100])
...
plot1("Repositioning Time (Minutes)")'''
...
as_ddqn = [74, 83.5, 90, 92, 92]
soft_cdqn = [75, 86.7, 92, 93, 93.1]
...
soft_cdqn = [61.50, 69, 77, 82, 87.00]
#bsac_nn = [62.70, 70, 78, 85, 89.50]
bsac_rnn_pdsa = [64.20, 74.10, 82.70, 89.20, 92]
plot1("Service Rate (%)", [50, 60, 70, 80, 90, 100])
...
bsac_rnn_pdsa = [3.2, 2.5, 2.53, 2.3, 2.15]
plot1("Response Time (Minutes)")
...
plot2("Repositioning Time (Minute)")
